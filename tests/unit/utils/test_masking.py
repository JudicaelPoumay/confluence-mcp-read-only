...
class TestLogConfigParam:
    ...
    def test_none_param(self, mock_logger):
        ...
        log_config_param(mock_logger, "Confluence", "Projects Filter", None)
        mock_logger.info.assert_called_once_with(
            "Confluence Projects Filter: Not Provided"
        )
    ...
    def test_sensitive_param(self, mock_logger):
        ...
        log_config_param(
            mock_logger, "Confluence", "API Token", "abcdefghijklmnop", sensitive=True
        )
        mock_logger.info.assert_called_once_with(
            "Confluence API Token: abcd********mnop"
        )
    ...
    def test_log_config_param_masks_proxy_url(self, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="test-proxy-logger"):
            log_config_param(
                logger, "Confluence", "SOCKS_PROXY", proxy_url, sensitive=True
            )
        # Should mask the middle part of the URL, not show user:pass
        assert any(
            rec.message.startswith("Confluence SOCKS_PROXY: sock")
